...
function getMAC(content: string, interfaceName: string, matchIP: string) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    while (true) {
      ...
      if (
        !line ||
        MAC_OSX_START_LINE.exec(line) ||
        MAC_LINUX_START_LINE.exec(line)
      ) {
        i--;
...
export function mac(
  interfaceNameOrCallback: string | MacCallback,
  ...
  if (
    !process.env.CI &&
    (item.mac === "ff:00:00:00:00:00" || item.mac === "00:00:00:00:00:00")
  ) {
...
export function dns(
  filepathOrCallback: string | DnsCallback,
...
export function address(
  interfaceNameOrCallback: string | AddressCallback,
