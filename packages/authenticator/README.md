...
Package for creating two step verification

**Jump to** — [Install](#install) · [Two-Factor Usage](#two-factor) · [General Usage](#general-usage) · [Contributing](#contributing) · [License](#license)

## Install

```sh
npm install --save @sse-auth/authenticator
```
...
Use sseauth's key generator to get a key.

```js
var secret = sseauth.generateSecret();
...
var verified = sseauth.totp.verify({
  secret: base32secret,
  encoding: "base32",
  token: userToken,
});
...
```js
var sseauth = require("@sse-auth/authenticator");
```
...
var secret = sseauth.generateSecret({ length: 20 });
...
// `hotp` (i.e. sseauth.hotp()) and a `counter` is given in the options.
var token = sseauth.totp({
...
var tokenValidates = sseauth.totp.verify({
  secret: secret.base32,
  encoding: "base32",
  token: "123456",
  window: 6,
...
// time-step.
var tokenDelta = sseauth.totp.verifyDelta({
  secret: secret.base32,
  encoding: "base32",
  token: "123456",
  window: 2,
...
var tokenValidates = sseauth.hotp.verify({
  secret: secret.base32,
  encoding: "base32",
  token: "123456",
  counter: 123,
...
```js
var token = sseauth.hotp.verifyDelta({
  secret: secret.ascii,
  counter: 42,
  token: "123456",
  window: 10,
...
var counter42 = sseauth.hotp({ secret: secret, counter: 42 });
// => '566646'

// Get HOTP counter token at counter = 45
var counter45 = sseauth.hotp({ secret: secret, counter: 45 });
// => '323238'
...
```js
var verified = sseauth.totp.verifyDelta({
  secret: secret.ascii,
  token: "123456",
  window: 2,
});
...
var token1 = sseauth.totp({ secret: secret, time: 1453853945 }); // 625175
var token3 = sseauth.totp({ secret: secret, time: 1453854005 }); // 222636
...
We're very happy to have your contributions in sseauth.
...
- SSE World ([sseworld][sseworld])

## License

This project incorporates code from [passcode][], which was originally a
fork of sseauth, and [notp][], both of which are licensed under MIT.
...
[issues]: https://github.com/sse-auth/react-3.x.x
