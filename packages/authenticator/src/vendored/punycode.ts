...
function mapDomain(domain: string, callback: (char: string) => string): string {
  ...
  let result = "";
  if (parts.length > 1) {
    // In email addresses, only the domain name should be punycoded. Leave the local part (i.e. everything up to `@`) intact.
    result = parts[0] + "@";
    domain = parts[1];
  }
  ...
  while (counter < length) {
    ...
    if (value >= 0xd800 && value <= 0xdbff && counter < length) {
      ...
      if ((extra & 0xfc00) === 0xdc00) {
        ...
        // It's an unmatched surrogate; only append this code unit, in case the next code unit is the high surrogate of a surrogate pair.
        output.push(value);
        counter--;
      }
    } else {
 ...
 * @returns {Number} The basic code point whose value (when used for representing integers) is `digit`, which needs to be in the range `0` to `base - 1`. If `flag` is non-zero, the uppercase form is used; else, the lowercase form is used. The behavior is undefined if `flag` is non-zero and `digit` has no uppercase form.
 */
 ...
 * Only the Punycoded parts of the input will be converted, i.e. it doesn't matter if you call it on a string that has already been converted to Unicode.
 * @memberOf punycode
