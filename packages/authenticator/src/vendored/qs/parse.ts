...
const parseValues = (
  ...
  if (options.throwOnLimitExceeded && parts.length > limit) {
    throw new RangeError(
      `Parameter limit exceeded. Only ${limit} parameter${
        limit === 1 ? "" : "s"
      } allowed.`
    );
  }
  ...
  for (i = 0; i < parts.length; ++i) {
    ...
    let val: any;
    if (pos === -1) {
      key = options.decoder(part, charset, "key");
      val = options.strictNullHandling ? null : "";
    } else {
      key = options.decoder(part.slice(0, pos), charset, "key");
      ...
      val = utils.maybeMap(
        ...
        (encodedVal) => options.decoder(encodedVal, charset, "value")
      );
    }
...
const parseQueryString = (
  ...
  let obj: Record<string, any> = options.plainObjects
    ? { __proto__: null }
    : {};
